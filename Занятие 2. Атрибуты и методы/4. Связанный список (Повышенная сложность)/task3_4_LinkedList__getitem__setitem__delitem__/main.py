...
class LinkedList:
    ...
    def __getitem__(self, index: int) -> Any:
        ...
        node = self.step_by_step_on_nodes(index)  # TODO получите узел при помощи метода step_by_step_on_nodes по требуемому индексу
        return node  # TODO верните значение узла

    def __setitem__(self, key: int, value: Any):
        """ Метод устанавливает значение узла по указанному индексу. """
        print(f"Вызов метода __setitem__, запросили изменение на позиции key={key} со значением value={value}")
        node = self.step_by_step_on_nodes(key)  # TODO получите узел при помощи метода step_by_step_on_nodes по требуемому индексу
        node.value = value  # TODO в атрибут value объекта node установите новое значение
    ...
    def __delitem__(self, key: int):
        ...
        if key == 0:
            ...
            self.head = self.step_by_step_on_nodes(1)  # TODO установите значение ссылки на следующий элемент
        else:
            # Поиск предыдущего узла
            previous_node = self.step_by_step_on_nodes(key-1)  # TODO найдите предыдущий узел (предыдущий относительно key)
            # Удаление текущего узла и связывание предыдущего с последующим
            current_node = self.step_by_step_on_nodes(key)  # TODO получите значение текущего узла через предыдущий
            # Следующий узел определяется относительно текущего
            next_node = current_node.next if current_node else None
            self.linked_nodes(previous_node, next_node)  # TODO свяжите предыдущий узел (previous_node) и следующий (next_node)  используя linked_nodes

        self.len -= 1  # TODO обновите значение длины после удаления
