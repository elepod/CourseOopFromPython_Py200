...
class LinkedList:
    ...
    def insert(self, index: int, value: Any) -> None:
        ...
        if index < 0:
            ...
            current_node = self.step_by_step_on_nodes(self.len - 1)
            current_node.next = Node(value)
        else:
            current_node = self.step_by_step_on_nodes(index)
            prev_node = self.step_by_step_on_nodes(index - 1)
            new_current_node = Node(value)
            self.linked_nodes(prev_node, new_current_node)
            self.linked_nodes(new_current_node, current_node)
            for i in range(index + 1, self.len - 1):
                self.linked_nodes(self.step_by_step_on_nodes(i), self.step_by_step_on_nodes(i + 1))
        self.len += 1

    def index(self, value: Any) -> int:
        """Возвращает индекс первого узла со значением, равным value."""
        for ind in range(self.len):
            if self.step_by_step_on_nodes(ind).value == value:
                return ind
    ...
    def pop(self, index: int = None) -> Any:
        ...
        if index and index < 0:
            raise ValueError("Element to delete can't have index less than 0")
        elif index and index >= self.len:
            raise ValueError("Element to delete can't have index less than 0")
        elif not index:
            pop_node = self.step_by_step_on_nodes(self.len - 1)
            self.step_by_step_on_nodes(self.len - 2).next = None
            self.len -= 1
            return pop_node
        else:
            pop_node = self.step_by_step_on_nodes(index)
            self.linked_nodes(self.step_by_step_on_nodes(index - 1),
                              self.step_by_step_on_nodes(index + 1))
            for i in range(index + 1, self.len - 1):
                self.linked_nodes(self.step_by_step_on_nodes(i), self.step_by_step_on_nodes(i + 1))
            self.len -= 1
            return pop_node
